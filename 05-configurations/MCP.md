...
## Shadcn-UI Integration (Modern React Components)

**Purpose**: Professional shadcn/ui v4 component integration, blocks, and design system implementation

**Activation Patterns**:
- Automatic: shadcn/ui references, modern React component requests, design system queries
- Manual: `--shadcn` flag  
- Smart: Frontend persona active, component library requests, UI modernization

**Workflow Process**:
1. Component Analysis: Use `list_components` to identify available shadcn/ui options
2. Requirements Matching: Match user requirements to appropriate components or blocks
3. Source Retrieval: Use `get_component` to fetch latest component source code
4. Demo Integration: Use `get_component_demo` for proper usage patterns and examples
5. Block Implementation: Use `get_block` for complete interface implementations (dashboards, forms)
6. Metadata Integration: Use `get_component_metadata` for dependencies and configuration
7. TypeScript Enhancement: Apply proper typing and interface definitions
8. Accessibility Validation: Ensure WCAG compliance and semantic markup
9. Responsive Implementation: Apply mobile-first responsive design patterns
10. Theme Integration: Adapt to existing Tailwind configuration and design tokens

**Component Categories**:
- Form Elements: Button, Input, Form, Select, Checkbox, RadioGroup, Switch
- Layout: Card, Sheet, Dialog, Popover, Tabs, Accordion, Separator
- Navigation: DropdownMenu, MenuBar, NavigationMenu, Breadcrumb, Pagination
- Feedback: Alert, Toast, Progress, Badge, Skeleton, Spinner
- Display: Table, Avatar, AspectRatio, Calendar, Carousel, Charts
- Interactive: Command, ComboBox, ContextMenu, HoverCard, Tooltip

**Block Categories**:
- Dashboards: dashboard-01 (admin), dashboard-02 (analytics), dashboard-03 (ecommerce)
- Authentication: authentication-01 (modern login), authentication-02 (signup flow)
- Forms: contact-01, newsletter-01, checkout-01
- Content: blog-01, portfolio-01, landing-01
- Communication: mail-01 (email client), chat-01 (messaging)

**Rate Limit Management**:
- GitHub PAT configured for 5,000 requests/hour
- Intelligent caching for repeated component requests
- Automatic fallback to documentation when limits approached
...
**Activation Patterns**:
- Automatic: UI component requests, design system queries (when shadcn/ui not specified)
- Manual: `--magic` flag
- Smart: Frontend persona active, custom component needs
